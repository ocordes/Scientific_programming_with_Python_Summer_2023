...
# changed by: Oliver Cordes 2023-06-20

# The twodanimation object is a simple object to store for a
...
#
#
class twodanimation(object):
    # __init__
    #
    # initialize the object, with the number of objects
    def __init__(self, nrobjects=1, figsize=(5,5), xlimits=(-10,10), ylimits=(-10,10), callback=None):
        self._time = []
    ...
    # args is a list which has a vector (x,y) for each point
    def append(self, time, args):
        self._time.append(time)
        
        if len(args) < (self._nrobjects):
            raise ValueError('Too few arguments in append.')
        
        # copy the values into the array
        for i in range(self._nrobjects):
            self._data[i]['x'].append(args[i][0])
            self._data[i]['y'].append(args[i][1])                          
